...
exports.getRestaurantByIdSpecialite = function(restaurantId, callback) {
  connection.query(
    "SELECT restaurant.idRestaurant, nomRestaurant, descriptionRestaurant, nombrePlaces, logoRestaurant, ville.idVille, nomVille, specialite.idSpecialite, specialite.libSpecialite, specialite.descSpecialite FROM restaurant INNER JOIN ville ON restaurant.idVille = ville.idVille LEFT JOIN proposer ON restaurant.idRestaurant = proposer.idRestaurant LEFT JOIN specialite ON specialite.idSpecialite = proposer.idSpecialite   WHERE restaurant.idRestaurant = " +
      restaurantId,
    function(error, results, fields) {
      if (error) throw error;
      callback(null, results);
